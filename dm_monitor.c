...
#define DM_MSG_PREFIX "dm_monitor"
...
static int monitor_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
...
// Optimization... Inline because this function is called in the loop :-)
...
static int monitor_map(struct dm_target *ti, struct bio *bio)
{
...
static void monitor_dtr(struct dm_target *ti) 
{
	struct target_info *target = ti->private;
...
static int monitor_end_io(struct dm_target *ti,
                     struct bio *bio,
                     blk_status_t *error)
{
	struct target_info *target = ti->private;
...
static struct target_type monitor_target = {
	.name   = "dm_monitor",
	.version = {1, 0, 0},
	...
	.dtr    = monitor_dtr,
	.map    = monitor_map,
	.end_io = monitor_end_io,
};
module_dm(monitor);

MODULE_AUTHOR("Slava Rybin");
MODULE_DESCRIPTION(DM_NAME " monitor target that checks data racing");
MODULE_LICENSE("GPL");
